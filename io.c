...
int readMatrixByRows(char* name, double *a, int matrix_side, int block_side, int total_pr, int current_pr){
   	FILE *fp = 0;
   	int ret=0, loc=0;
   	int i, j, count=0;
	double trash;
...
int readMatrixByColumns(char* name, double *a, int matrix_side, int block_side, int total_pr, int current_pr){
    FILE *fp = 0;
   	int ret=0, loc=0;
	int i, j;
	int count = 0;
...
void MPI_printUpperLeftBlock(double *a, int matrix_side, int block_side, int total_pr, int current_pr, const int *blocks_order_reversed, 
	...
	buf = (double*) malloc(corner_side*block_string_size*sizeof(double)); 

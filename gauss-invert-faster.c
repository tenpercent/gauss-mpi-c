...
int gaussInvert(double *a, double *b, int matrix_side, int block_side, 
  ...
  MPI_Status status;
  int start_nonzero_a=0;
  int nonzero_a_size;
  int local_index_for_swap;
    ...
    if (current_pr==out.rank){
      ...
      if(small_block_row_width){
		...
		if (out.rank!=first_row_proc_id){
			if (current_pr==first_row_proc_id){
				for (j=0; j<block_string_size; j++){
          local_index_for_swap = first_row*block_string_size+j;
					buf_string_2[j]=b[local_index_for_swap];
          b[local_index_for_swap]=buf_string[j];
				}
				for (j=start_nonzero_a, k=0; j<block_string_size; j++, k++){
          local_index_for_swap = first_row*block_string_size+j;
					buf_string_2[k+block_string_size]=a[local_index_for_swap];
          a[local_index_for_swap]=buf_string[k+block_string_size];
				}
